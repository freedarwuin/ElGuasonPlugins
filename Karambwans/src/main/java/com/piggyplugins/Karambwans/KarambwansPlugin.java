...
import static com.piggyplugins.Karambwans.KarambwansState.*;
...
public class KarambwansPlugin extends Plugin {
    ...
    @Inject
    KarambwansConfiguration config;
    @Inject
    private OverlayManager overlayManager;
    @Inject
    private ConfigManager configManager;
    @Inject
    private KarambwansOverlay overlay;
    @Inject
    ...
    KarambwansState state;
    int timeout = 0;
    private int nextRunEnergy;
    List<WorldPoint> FISH_AREA = (new WorldArea(2893,3111, 40, 15, 0).toWorldPointList());

    @Provides
    KarambwansConfiguration provideConfig(ConfigManager configManager) {
        return configManager.getConfig(KarambwansConfiguration.class);
    }
    ...
    public KarambwansState getState() {
    ...
    private void onGameTick(GameTick tick) {
        ...
        switch (state) {
            ...
            case DEPOSIT_FISH:
                ...
                Optional<Widget> fish = BankInventory.search().withId(ItemID.RAW_KARAMBWAN).first();
                Optional<Widget> barrel = BankInventory.search().withId(ItemID.OPEN_FISH_BARREL).first();

                if (barrel.isPresent()) {
                    BankInventoryInteraction.useItem(barrel.get(), "Empty");
                }
                BankInventoryInteraction.useItem(fish.get(), "Deposit-All");
                timeout = tickDelay();
                break;
    ...
    public void handleFairyRing() {
        Optional<TileObject> fairy_ring = TileObjects.search().withName("Fairy ring").nearestToPlayer();
        if (fairy_ring.isPresent()) {
            ...
            //Failsafe in case fairy ring isn't visible or in distance. Prolly not needed tho.
    ...
    public void handleFish() {
        Optional<NPC> fish = NPCs.search().withName("Fishing spot").nearestToPlayer();
